...
if (logo_header && logo_footer) {
  for (let logo of logo_header) {
   ...
   fs.renameSync(logo.path, path.resolve(config.get("app.baseUrlUpload"),logoHeader))
}
for (let logo of logo_footer) {
 ...
 fs.renameSync(logo.path, path.resolve(config.get("app.baseUrlUpload"),logoFooter))
